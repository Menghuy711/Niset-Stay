...
export default function Footer() {
  ...
  const isHidden = (to) => {
    if (role === 'landlord') return to === '/rent' || to === '/news';
    if (['admin', 'super_admin'].includes(role)) return to === '/rent' || to === '/news' || to === '/about';
    return false;
  };
        ...
        <div className="container">
          ...
          <nav className="footer-nav" aria-labelledby="nav-label-1">
            ...
            <ul className="footer-list">
              ...
              {!isHidden('/rent') && <li><Link to="/rent" className="body-medium footer-link">Rent</Link></li>}
              {!isHidden('/news') && <li><Link to="/news" className="body-medium footer-link">News &amp; Events</Link></li>}
              {!isHidden('/about') && <li><Link to="/about" className="body-medium footer-link">About Us</Link></li>}
              {role === 'landlord' && (
                <li><Link to="/landlord" className="body-medium footer-link">Landlord Portal</Link></li>
              )}
              {['admin', 'super_admin'].includes(role) && (
                <li>
                  <Link to="/admin" className="body-medium footer-link">
                    {role === 'super_admin' ? 'Super Admin' : 'Admin'}
                  </Link>
                </li>
              )}
              {role === 'student' && (
                <li><Link to="/my-bookings" className="body-medium footer-link">My Bookings</Link></li>
              )}
              {role && (
                <li><Link to="/profile" className="body-medium footer-link">Profile</Link></li>
              )}
            </ul>
          </nav>
          ...
          <nav className="footer-nav" aria-labelledby="nav-label-3">
            ...
            <ul className="footer-list">
              <li><a href="mailto:houseandroom@nisetstay.com" className="body-medium footer-link">houseandroom@nisetstay.com</a></li>
              <li>
